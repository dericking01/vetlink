...
<div class="card">
    <div class="card-header bg-light">
      <div class="row align-items-center">
          <div class="col">
              <h5 class="mb-0" id="followers">Paid Orders
          ...
          <table class="table data-table table-bordered table-striped fs--1 mb-0">
            ...
            <tbody class="list">
              ...
              <tr>
                ...
                <td class="buyer">{{ $invoice->order->user->name }}</td>
                <td class="agent">{{ $invoice->order->agent->name }}</td>

                <td class="quantity">{{$invoice->TotalQty}}</td>
                <td class="price">{{ $invoice->TotalAmt }}</td>
                <td class="discount">{{ $invoice->discount }}</td>
                <td class="status text-center">
                    <span class="badge badge-subtle-success">{{ $invoice->status }}</span>
                </td>
                <td class="align-middle white-space-nowrap text-end">
                  <div class="dropstart font-sans-serif position-static d-inline-block">
                      ...
                      <div class="dropdown-menu dropdown-menu-end border py-2"
                        ...
                        <a class="dropdown-item text-primary" href="{{ route('seller.viewOrder', $invoice->order->id) }}">View</a>
                      </div>
                    </div>
                </td>
              </tr>
