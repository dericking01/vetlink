...
<div class="card">
  <div class="card-header bg-light">
    <div class="row align-items-center">
        <div class="col">
            <h5 class="mb-0" id="followers">Sellers
              <span class="d-none d-sm-inline-block">({{ $sellers->count() }})</span>
                  </h5>
        </div>
        ...
        <table class="table data-table table-bordered table-striped fs--1 mb-0">
          <thead class="bg-200 text-900">
            <tr>
              ...
              <th class="sort pe-1 align-middle white-space-nowrap">Seller's Name</th>
          ...
          <tbody class="list">
            @foreach($sellers as $key => $seller)
              <tr class="btn-reveal-trigger">
                <td class="sn">{{ ++$key }}</td>
                <td class="name align-middle white-space-nowrap py-2"><a href="{{ route('seller.view', $seller->id) }}">
                  <div class="d-flex d-flex align-items-center">
                        <div class="avatar avatar-xl me-2">
                        <div class="avatar-name rounded-circle"><span>{{ \App\Helpers\SettingsHelper::getSellerInitials($seller->id) }}</span></div>
                        </div>
                        <div class="flex-1">
                        <h5 class="mb-0 fs--1">{{ $seller->name }}</h5>
                        </div>
                    </div>
                  </a>
                </td>
                <td class="joined align-middle py-2">{{date_format(date_create($seller->created_at), 'd M, Y')}}</td>
                <td class="phone align-middle white-space-nowrap py-2 text-center">
                      {{$seller->phone}}
                  </td>
                  <td class="align-middle text-center">
                    @if($seller->status == 'Pending')
                      <small class="badge fw-semi-bold rounded-pill badge-subtle-info">PENDING</small>
                  ...
                  <td class="align-middle text-center">
                    <small class="badge fw-semi-bold rounded-pill badge-subtle-primary">SELLER</small>
                  </td>
                  <td class="align-middle white-space-nowrap py-2 text-center">
                      ...
                      <div class="dropdown-menu dropdown-menu-end border py-0" aria-labelledby="customer-dropdown-0">
                        <div class="py-2"><a class="dropdown-item text-success" href="#!" data-bs-toggle="modal" data-bs-target="#editSeller{{ $seller->id }}" >Edit</a></div>
                        <div class="py-2"><a class="dropdown-item text-danger" href="#!" data-bs-toggle="modal" data-bs-target="#deleteSeller{{ $seller->id }}" >Delete</a></div>
                      </div>
                      </div>
                  </td>
              </tr>
              ...
              {{-- edit seller --}}
              <div class="modal fade" id="editSeller{{ $seller->id }}" tabindex="-1" role="dialog" aria-hidden="true">
                          ...
                          <div class="modal-content position-relative">
                            ...
                            <div class="modal-body p-0">
                                <div class="rounded-top-3 py-3 ps-4 pe-6 bg-light">
                                    <h4 class="mb-1" id="modalExampleDemoLabel">Edit Seller </h4>
                                </div>
                                  ...
                                  <div class="row">
                                      <div class="col-md-12">
                                          <div class="mb-3">
                                            ...
                                            <input class="form-control @error('name') is-invalid @enderror" id="validationCustom03" value="{{ old('name', $seller->name) }}" name="name" type="text" autocomplete="on" placeholder="Full Name" readonly/>
                                            @error('name')
                                      ...
                                      <div class="col-md-12">
                                        <div class="mb-3">
                                          ...
                                          <input class="form-control @error('phone') is-invalid @enderror" id="validationCustom04" value="{{ old('phone', $seller->phone) }}" type="phone" name="phone" autocomplete="on" placeholder="Phone number" readonly/>
                                          @error('phone')
                                              <div class="invalid-feedback">{{ $message }}</div>
                                          @enderror
                                        </div>
                ...
                <div class="modal fade" id="deleteSeller{{ $seller->id }}" tabindex="-1" role="dialog" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered" role="document" style="max-width: 500px">
                        <div class="modal-content">
                            ...
                            <form action="{{ route('seller.destroyseller') }}" method="POST">
                                @csrf
                                @method('DELETE')
